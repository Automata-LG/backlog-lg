...
```json
{
  "list": [
    {
      "sn": "COUPON-123456", // nullable - 쿠폰 번호 (가상 쿠폰은 발급 시점엔 null)
      "status": "ISSUED", // 쿠폰 상태
      "issuedAt": "2025-01-01T00:00:00Z", // nullable - 발급일
      "redeemedAt": null, // nullable - 사용일
      "startDate": "2025-01-01T00:00:00Z", // nullable - 시작일
      "endDate": "2025-12-31T23:59:59Z", // nullable - 종료일
      "coupon": {
        "id": "uuid-here", // nullable - 쿠폰 ID (외부 시스템 쿠폰만 null)
        "kind": "BESTSHOP_FREE_PARKING", // 쿠폰 종류
        "name": "베스트샵 무료 주차 2시간", // 쿠폰명
        "displayName": "베스트샵 무료 주차권", // nullable - 화면 표시용 이름
        "description": "베스트샵 방문 시 사용 가능한 2시간 무료 주차권", // nullable - 쿠폰 설명
        "discountType": "PASS", // 할인 타입
        "discountValue": "0", // 할인 값
        "maxDiscount": "0" // 최대 할인 금액
      },
      "reservation": null // nullable - 예약 정보 (예약 연결 시 {id: "uuid"})
    }
  ]
}
```
...
데이터베이스의 Coupon 모델 정의:

```prisma
model Coupon {
  id            String             @id @default(dbgenerated("gen_random_uuid()")) @db.Uuid
  kind          CouponKind
  name          String             @db.VarChar(255)          // 쿠폰명
  displayName   String?            @db.VarChar(255)          // 화면 표시용 이름 (선택적)
  description   String?                                      // 쿠폰 설명 (선택적)
  images        String[]           @db.VarChar(2048)         // 이미지 URL 배열
  instructions  String?                                      // 사용 안내 (선택적)
  cautions      String?                                      // 주의사항 (선택)
  issuanceType  CouponIssuanceType
  discountType  CouponDiscountType                          // AMOUNT, RATIO, PASS
  discountValue Decimal            @default(0) @db.Decimal(19, 4)
  maxDiscount   Decimal            @default(0) @db.Decimal(19, 4)
  policy        Json               @default("{}")

  startDate DateTime                                         // 쿠폰 시작일
  endDate   DateTime                                         // 쿠폰 종료일
...
- **id**: 고유 식별자 (UUID)
- **kind**: 쿠폰 종류 (CouponKind enum)
- **name**: 쿠폰명
- **displayName**: UI에 표시될 화면용 이름 (nullable, name과 병행 사용)
- **description**: 쿠폰에 대한 상세 설명 (nullable)
- **images**: 쿠폰 이미지 URL 배열
- **instructions**: 쿠폰 사용 방법 안내 (nullable)
- **cautions**: 주의사항 (선택 사항)
- **issuanceType**: 발급 타입
- **discountType**: 할인 타입 (금액/비율/패스)
- **discountValue**: 할인 값
- **maxDiscount**: 최대 할인 금액
- **policy**: 추가 정책 정보 (JSON)
- **startDate/endDate**: 쿠폰 유효 기간
...
| GET    | `/external/app/coupon/base` | 베이스 쿠폰 조회  | 불필요 |
    ...
    "list": [
      {
        ...
        "endDate": "2025-12-31T23:59:59Z", // nullable - 종료일
        "coupon": {
          "id": null, // nullable - 쿠폰 ID
          "kind": "BESTSHOP_FREE_PARKING", // 쿠폰 종류
          "name": "베스트샵 무료 주차 2시간", // 쿠폰명
          "displayName": "베스트샵 무료 주차권", // nullable - 화면 표시용 이름
          "description": null, // nullable - 쿠폰 설명
          "discountType": "PASS", // 할인 타입
          "discountValue": "0", // 할인 값
          "maxDiscount": "0" // 최대 할인 금액
        },
        "reservation": null // nullable - 예약 정보
      },
      {
        "sn": null, // nullable - 가상 쿠폰은 발급 시점엔 null
        "status": "ISSUED",
        "issuedAt": null, // nullable
        "redeemedAt": null, // nullable
        "startDate": "2025-01-01T00:00:00Z", // nullable - 등급 시작일
        "endDate": "2025-12-31T23:59:59Z", // nullable - 등급 종료일
        "coupon": {
          "id": "660e8400-e29b-41d4-a716-446655440001", // nullable - 쿠폰 ID
          "kind": "SKS_DINING_SIGNATURE_BEVERAGE_FREE",
          "name": "SKS 시그니처 음료", // 쿠폰명
          "displayName": "SKS 시그니처 음료 무료", // nullable - 화면 표시용 이름
          "description": null, // nullable - 쿠폰 설명
          "discountType": "FREE",
          "discountValue": "0",
          "maxDiscount": "0"
        },
        "reservation": null // nullable
      },
      {
        "sn": "LGC-2025-00123", // 사용 시 생성된 쿠폰 번호
        "status": "REDEEMED",
        "issuedAt": "2025-02-15T10:00:00Z",
        "redeemedAt": "2025-02-15T10:00:00Z",
        "startDate": "2025-01-01T00:00:00Z",
        "endDate": "2026-12-31T23:59:59Z",
        "coupon": {
          "id": "770e8400-e29b-41d4-a716-446655440002", // 쿠폰 ID
          "kind": "PREMIUM_LOUNGE_ACCESS",
          "name": "프리미엄 라운지 이용권", // 쿠폰명
          "displayName": "프리미엄 라운지", // nullable - 화면 표시용 이름
          "description": null, // nullable - 쿠폰 설명
          "discountType": "PASS",
          "discountValue": "0",
          "maxDiscount": "0"
        ...
        "reservation": {
          "id": "990e8400-e29b-41d4-a716-446655440999" // 연결된 예약 ID
        }
      }
    ]
  }
...
#### 베이스 쿠폰 조회 (couponBase)

현재 유효한 모든 베이스 쿠폰의 상세 정보를 조회합니다.
...
- **응답**: 200 OK
  ```json
  {
    "list": [
      {
        "id": "880e8400-e29b-41d4-a716-446655440003", // 쿠폰 ID
        "kind": "SKS_DINING_MEAL_DISCOUNT",
        "name": "SKS 다이닝 식사권", // 쿠폰명
        "displayName": "SKS 다이닝 할인", // nullable - 화면 표시용 이름
        "description": "\"SKS로 완성하는 미식의 여정\"\n\nSKS Dining은...", // nullable - 쿠폰 설명
        "images": ["https://example.com/image1.jpg", "https://example.com/image2.jpg"],
        "instructions": "•식사는 런치 시간대에만 운영되며...",
        "cautions": "•예약 변경 및 취소는 방문 24시간 전까지 가능합니다.", // nullable - 주의사항
        "policy": {
          "applicableTo": {
            "displayText": "SKS 다이닝 레스토랑 이용 시",
            "rules": {
              "location": "SKS_DINING",
              "usagePeriod": "MEMBERSHIP_YEAR"
            }
          }
        },
        "discountType": "RATE",
...
| GET    | `/external/admin/coupon/base`        | 베이스 쿠폰 조회    | 필요 |
...
#### 베이스 쿠폰 조회 (couponBase) - 관리자용

현재 유효한 모든 베이스 쿠폰의 상세 정보를 조회합니다.

- **요청**: 파라미터 없음

- **응답**: 앱 API의 couponBase와 동일한 형식
