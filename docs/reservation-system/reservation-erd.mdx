...
erDiagram
    ...
    ReservationSpace ||--o{ ReservationUnit : "has"
    ReservationSpace ||--o{ ReservationContent : "has"
    ReservationSpace ||--o{ Reservation : "contains"
    ReservationUnit ||--o{ Reservation : "is booked"
...
2. **ReservationSpace** (예약 공간)
   - 호스트 내의 개별 공간 (예: 공연장, 전시실, 다이닝홀, 룸)
   - 하나의 호스트에 여러 공간 존재 가능
   - Unit들과 Reservation들을 포함

3. **ReservationUnit** (예약 단위)
   - 실제 예약 가능한 최소 물리적 단위 (예: 좌석, 테이블)
   - `sn`: 고유 좌석/테이블 코드 (예: "R1", "H4-1")
   - 최소/최대 수용 인원 (`minCapacity`/`maxCapacity`) 관리
   - 메타데이터로 추가 속성 저장 가능
   - Fractional Index로 순서 관리

4. **ReservationContent** (예약 콘텐츠)
