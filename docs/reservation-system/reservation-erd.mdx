...
erDiagram
    ...
    Member ||--o{ ReservationWaitlist : "joins"
    Coupon ||--o{ CouponPool : "has"
    CouponPool ||--o{ CouponPoolHist : "has"
    Admin ||--o{ ReservationHist : "creates"
    ReservationContent ||--o{ ReservationWaitlist : "has waitlist for"
    ReservationTimeSlot }o--o{ ReservationWaitlist : "may have waitlist"
```
...
model ReservationWaitlist {
  ...
  @@unique([memberId, contentId, timeSlotId, type])
  @@index([timeSlotId, type])
  @@index([contentId, type])
  @@map(name: "reservationWaitlist")
}
```

**주요 특징**:
- `type`: 대기 유형 (VACANCY: 빈자리 알림, OPEN: 오픈 알림)
- `timeSlotId`: VACANCY 타입에서만 사용, OPEN 타입에서는 null
- 유니크 제약: 동일한 회원이 같은 대상에 중복 신청 불가
- 인덱스: 타임슬롯/컨텐츠별 빠른 조회를 위한 인덱스
