...
```json
{
  ...
  "data_schema": {
    "variables": {
      ...
      "식사/음료": { "type": "string", "required": true, "enum": ["식사", "음료"] }
    }
  },
  "default_delay": 0,
  "target_subscription_types": ["ios_push", "android_push", "kakao"]
}
```

---

### 2. SKS Dining 예약 리마인더 D-1 (`reservation.reminder_d1`)
...
```json
{
  "event_key": "reservation.reminder_d1",
...
### 3. SKS Dining 예약 리마인더 D-0 (`reservation.reminder_d0`)
...
```json
{
  "event_key": "reservation.reminder_d0",
...
### 4. SKS Dining 예약 취소 - 고객 (`reservation.cancelled_user`)
...
```json
{
  "event_key": "reservation.cancelled_user",
...
### 5. SKS Dining 예약 취소 - 관리자 (`reservation.cancelled_admin`)
...
```json
{
  "event_key": "reservation.cancelled_admin",
