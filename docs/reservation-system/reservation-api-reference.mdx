...
| `FDE-A-002` | `[유효한 방문 일시를 찾을 수 없습니다]` | 잘못된 타임슬롯 ID 또는 비활성화된 타임슬롯 |
...
| `FDE-A-002` | `[{등급} 등급 이상의 회원만 이용할 수 있습니다]` | 최소 회원 등급 미달 (restrictionConfig.minMembershipGrade) |
| `FDE-A-002` | `[해당 예약은 {최소}명에서 {최대}명까지 이용 가능합니다]` | minGroupSize ~ maxGroupSize 범위 초과 |
| `FDE-A-002` | `[예약이 불가합니다. 다시 확인해 주세요]` | FFD 시뮬레이션 결과 섹션/유닛 배치 불가능 |
| `FDE-A-002` | `[예약 정원을 초과하였습니다]` | 타임슬롯 총 정원 초과 |
...
| `FDE-A-002` | `[해당 예약은 {최소}명에서 {최대}명까지 이용 가능합니다]` | 인원 변경 시 범위 초과 |
| `FDE-A-002` | `[예약이 불가합니다. 다시 확인해 주세요]` | 타임슬롯/인원 변경 시 수용 불가 |
