...
| HTTP 코드 | 에러 메시지 | 발생 조건 |
|----------|------------|----------|
| `401` | `FDE-A-001` | JWT 토큰 없음 또는 만료 |
| `404` | `FDE-C-002 [회원 정보를 찾을 수 없습니다]` | 회원을 찾을 수 없음 (NewBest API에서 memberCi로 조회 실패) |
| `403` | `FDE-A-002 [이미 해당 시간에 예약이 있습니다]` | 동일 타임슬롯에 중복 예약 시도 (게스트 제외) |
| `403` | `FDE-A-002 [유효한 방문 일시를 찾을 수 없습니다]` | 잘못된 타임슬롯 ID 또는 비활성화된 타임슬롯 |
| `403` | `FDE-A-002 [현재 예약 가능한 시간이 아닙니다]` | 예약 오픈 시간 범위를 벗어남 (BATCH/SEQUENTIAL/GRADED 타입별 기준) |
| `403` | `FDE-A-002 [{쿠폰명} 이용 횟수 초과 (X/Y회)]` | 연간 멤버십 쿠폰 사용 횟수 초과 |
| `403` | `FDE-A-002 [필수 약관에 모두 동의해야 합니다]` | 필수 약관 미동의 |
| `403` | `FDE-A-002 [{등급} 등급 이상의 회원만 이용할 수 있습니다]` | 최소 회원 등급 미달 (restrictionConfig.minMembershipGrade) |
| `403` | `FDE-A-002 [같은 날에는 최대 {N}회까지만 예약 가능합니다]` | 같은 이용일에 중복 예약 시도 (restrictionConfig.maxReservationPerDay) |
| `403` | `FDE-A-002 [해당 예약은 {최소}명에서 {최대}명까지 이용 가능합니다]` | minGroupSize ~ maxGroupSize 범위 초과 |
| `403` | `FDE-A-002 [예약이 불가합니다. 다시 확인해 주세요]` | FFD 시뮬레이션 결과 섹션/유닛 배치 불가능 |
| `403` | `FDE-A-002 [예약 정원을 초과하였습니다]` | 타임슬롯 총 정원 초과 |
| `403` | `FDE-A-002 [노쇼 패널티로 인해 예약이 제한되어 있습니다. (해제일: {날짜})]` | SKS 다이닝 예약 시 6개월 이내 노쇼 기록 존재 |
| `409` | `FDE-C-003` | Unique constraint 위반 (기타 중복 데이터) |
| `409` | `FDE-C-004` | Foreign key constraint 위반 (잘못된 참조) |
| `500` | `FDE-C-101` | 데이터베이스 내부 에러 |
| `500` | `FDE-C-901,1` | NewBest API 연결 실패 |
| `500` | `FDE-C-901,2 [{resultCode}: {resultMsg}]` | NewBest API 응답 에러 |
...
| HTTP 코드 | 에러 메시지 | 발생 조건 |
|----------|------------|----------|
| `400` | `FDE-C-001 [유닛 배정 전에는 인원 변경이 불가능합니다. 예약을 취소 후 다시 생성해 주세요.]` | 유닛 미배정 상태에서 groupSize 변경 시도 |
| `401` | `FDE-A-001` | JWT 토큰 없음 또는 만료 |
| `403` | `FDE-A-002 [수정 불가능한 예약입니다]` | 이미 취소됨, 이용완료, 노쇼 처리됨, 또는 수정 마감 시간 경과 |
| `403` | `FDE-A-002 [예약 가능 시간이 지났습니다]` | 타임슬롯 종료 시간 경과 (어드민용) |
| `403` | `FDE-A-002 [현재 예약 가능한 시간이 아닙니다]` | 타임슬롯 변경 시 예약 오픈 시간 검증 실패 |
| `403` | `FDE-A-002 [해당 예약은 {최소}명에서 {최대}명까지 이용 가능합니다]` | 인원 변경 시 범위 초과 |
| `403` | `FDE-A-002 [예약이 불가합니다. 다시 확인해 주세요]` | 타임슬롯/인원 변경 시 수용 불가 |
| `404` | `FDE-C-002` | 예약을 찾을 수 없음 (예약 ID 또는 회원 ID 불일치) |
| `409` | `FDE-C-003` | Unique constraint 위반 |
| `409` | `FDE-C-004` | Foreign key constraint 위반 |
| `500` | `FDE-C-101` | 데이터베이스 내부 에러 |
| `500` | `FDE-C-901,1` | NewBest API 연결 실패 |
| `500` | `FDE-C-901,2 [{resultCode}: {resultMsg}]` | NewBest API 응답 에러 |
...
|----------|------------|----------|
| `401` | `FDE-A-001` | JWT 토큰 없음 또는 만료 |
| `403` | `FDE-A-002 [취소 불가능한 예약입니다]` | 이미 취소됨, 이용완료, 노쇼 처리됨, 또는 취소 마감 시간 경과 |
| `404` | `FDE-C-002` | 예약을 찾을 수 없음 (예약 ID 또는 회원 ID 불일치) |
...
| HTTP 코드 | 에러 메시지 | 발생 조건 |
|----------|------------|----------|
| `400` | `FDE-C-001 [아직 자리가 남아있습니다. 매진된 타임슬롯만 알림 신청이 가능합니다.]` | 매진되지 않은 타임슬롯에 신청 |
| `400` | `FDE-C-001 [해당 타임슬롯의 빈자리 대기 신청이 마감되었습니다. (최대 10명)]` | 대기자 수 10명 초과 |
| `401` | `FDE-A-001` | JWT 토큰 없음 또는 만료 |
| `404` | `FDE-C-002 [타임슬롯을 찾을 수 없습니다]` | 타임슬롯을 찾을 수 없음 |
...
3. **중복 예약 체크**:
   - 게스트(GUEST) 회원 제외
   - 일반 회원은 동일 타임슬롯에 PENDING/CONFIRMED 상태 예약 불가
4. 쿠폰 사용 가능 여부 확인 (프리미엄 라운지, SKS 다이닝 등)
5. `confirmConfig`에 따라 예약 상태 결정:
...
#### 중복 예약 방지
- **적용 대상**: 일반 회원 (게스트 회원 제외)
- **검증 시점**: 예약 생성 시 (수정 시는 제외)
- **검증 조건**:
  - 동일 회원 + 동일 타임슬롯
  - 상태가 PENDING 또는 CONFIRMED인 예약
- **게스트 예외**: 이름이 'GUEST'인 회원은 중복 체크 제외 (다중 예약 허용)

#### 노쇼 패널티 검증
- **적용 대상**: SKS 다이닝 (host.kind: RESTAURANT)
- **검증 시점**: 예약 생성 시
- **패널티 기간**: 노쇼 발생일로부터 6개월
- **에러 메시지**: "노쇼 패널티로 인해 예약이 제한되어 있습니다. (해제일: YYYY년 MM월 DD일)"
- **비적용 대상**: 아트센터, 프리미엄 라운지 등 다른 서비스

#### 기타 검증 사항
- 회원 등급 확인 (minMembershipGrade)
- 인원수 제한 (minGroupSize ~ maxGroupSize)
- 예약 오픈 시간 확인 (BATCH/SEQUENTIAL/GRADED)
- 쿠폰 사용 가능 여부 및 사용 횟수
- 필수 약관 동의 여부
- 타임슬롯 정원 초과 여부