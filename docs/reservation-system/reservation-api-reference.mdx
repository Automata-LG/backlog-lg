...
```typescript
{
  kind?: 'ART_CENTER' | 'RESTAURANT'  // [선택] 호스트 종류 필터
  page?: number                        // [선택] 페이지 번호 (기본값: 1)
  pageSize?: number                    // [선택] 페이지 크기 (기본값: 10, 최대: 500)
}
```
...
```typescript
{
  hostId?: string     // [선택] 호스트 ID 필터 (UUID)
  spaceId?: string    // [선택] 스페이스 ID 필터 (UUID)
  page?: number       // [선택] 페이지 번호 (기본값: 1)
  pageSize?: number   // [선택] 페이지 크기 (기본값: 10, 최대: 500)
}
```
...
```typescript
{
  contentId: string         // [필수] 컨텐츠 ID (UUID)
  startsAfter?: Date       // [선택] 시작일 이후 필터 (ISO 8601)
  startsBefore?: Date      // [선택] 시작일 이전 필터 (ISO 8601)
  page?: number            // [선택] 페이지 번호 (기본값: 1)
  pageSize?: number        // [선택] 페이지 크기 (기본값: 500, 최대: 500)
}
```
...
{
  contentId: string         // [필수] 컨텐츠 ID (UUID)
  kind?: 'MENU' | 'OPTION'  // [선택] 가격 항목 종류 필터
  page?: number            // [선택] 페이지 번호 (기본값: 1)
  pageSize?: number        // [선택] 페이지 크기 (기본값: 10, 최대: 500)
}
```
...
{
  hostId?: string       // [선택] 호스트 ID 필터 (UUID)
  isActive?: boolean    // [선택] 활성 상태 필터 (기본값: true)
  orderBy?: string      // [선택] 정렬 기준 (기본값: "createdAt_desc")
  page?: number         // [선택] 페이지 번호 (기본값: 1)
  pageSize?: number     // [선택] 페이지 크기 (기본값: 20, 최대: 500)
}
```
