...
관리자는 고객 서비스와 운영 효율성을 위해 일반 사용자보다 확장된 예약 관리 권한을 가집니다. 이는 현장 상황에 대한 유연한 대응과 VIP 고객 서비스를 위한 필수 기능입니다.
...
**인원(groupSize) 변경 특별 규칙**
- **유닛 배정 전**: 변경 불가
  - 에러 메시지: "유닛 배정 전에는 인원 변경이 불가능합니다. 예약을 취소 후 다시 생성해 주세요."
  - 인원 변경이 필요한 경우 예약 취소 후 재생성 필요
- **유닛 배정 후**: FFD 알고리즘이 실시간으로 가능 범위 자동 체크
  - 4인 테이블 (minCapacity 2, maxCapacity 4): 2~4명 범위 내에서만 변경 가능
  - 6인 테이블 (minCapacity 2, maxCapacity 6): 2~6명 범위 내에서만 변경 가능
  - 변경하려는 인원이 유닛 capacity를 벗어나면 자동으로 다른 유닛 재배치 시도
  - 재배치 불가능 시 "예약이 불가능합니다" 에러 발생
...
**특징**:
- 타임슬롯 종료 시간까지 생성 가능
- openConfig 무시하고 즉시 예약 가능

#### 2. reservationCreateForGuest
**용도**: 비회원 예약 생성
...
- 시간 제한 없이 수정 가능
- FSM 규칙에 따른 상태 전이 검증
- `resolveReservationStatusTransition` 함수 사용

#### 4. reservationStatusBulkChange
**용도**: 예약 상태 대량 변경
**엔드포인트**: `POST /external/admin/reservation/status/bulk-change`
**지원 상태**:
- `CONFIRMED`: 예약 확정
- `CANCELLED`: 예약 취소 (쿠폰 환불)
- `NOSHOW`: 노쇼 처리
- `USED`: 이용 완료
...
- 예약 변경 정책 구현 완료
  ...
  - FFD 알고리즘이 유닛 capacity 자동 체크하도록 명시
