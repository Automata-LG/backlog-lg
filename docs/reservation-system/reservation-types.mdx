...
<Note>
**additionalInfo 변경 추적 방식**
- `additionalInfo`는 오브젝트 타입으로, 부분 업데이트를 지원합니다
- 변경 이력에는 **실제로 변경된 필드만** 기록됩니다
- 예시:
  ```typescript
  // 원본
  additionalInfo: { guestName: "홍길동", specialRequest: "창가", memo: "VIP" }

  // 업데이트 (specialRequest만 변경)
  update: { specialRequest: "룸 배정" }

  // 기록되는 변경 내역
  detailedChanges.additionalInfo: {
    old: { specialRequest: "창가" },      // 변경된 필드만
    new: { specialRequest: "룸 배정" }    // 변경된 필드만
  }
  ```
</Note>
